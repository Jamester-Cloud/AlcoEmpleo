...
export async function middleware(request: NextRequest) {
  ...
  const isEnterprisePath =
    path === "/enterprise" ||
    path === "/enterprise/premium" ||
    path === "enterprise/jobOffer" ||
  ...
  const isCandidatePath =
    ...
    path === "/candidate/oportunidades" ||
    path === "candidate/quizz" ||
    path === "/candidate/quizz/[id]";
  ...
  const isAdminPath =
    ...
    path === "/admin/quizzes" ||
    path === "/admin/quizzes/psychotechnical";
  ...
  if (isAdminPath && token && tokenData.payload?.rol != "admin")
    return NextResponse.redirect(new URL("/", request.nextUrl));
  //Espacio solo candidatos
  if (isEnterprisePath && tokenData.payload?.rol === "Candidatos" && token) {
    return NextResponse.redirect(new URL("/candidate", request.nextUrl));
  }

  //Espacio solo empresa
  if (isCandidatePath && tokenData.payload?.rol === "Empresas" && token)
    return NextResponse.redirect(new URL("/enterprise", request.nextUrl));
...
export const config = {
  matcher: [
    ...
    "/admin",
    "/admin/quizzes/:id",
    "/candidate/oportunidades",
    "/candidate/quizz",
    "/candidate/quizz/:id",
    "/admin/quizzes/psychotechnical",
    "/admin/quizzes/:id"
  ],
};
