...
export default function Quizzes({ params }: any) {
  ...
  useEffect(() => {
    ...
    defaultValues.quiz = quiz?.map((item: any) => {
      return {
        pregunta: item.pregunta,
        respuestas: item.respuestas.map((item: any) => {
          return { respuesta: item.respuesta };
        }),
        respuestaCorrecta:
          item.respuestaCorrecta || item.respuestas[0].respuesta,
      };
    });
  ...
  const generateQuestions = async () => {
    ...
    try {
      const response = await axios.post(
        "/api/administrator/candidates/quizzes",
        { idCandidato: id, dificultad: dificultad }
      );
      ...
      toast.error(
        `Cuestionario no generado, error es: ${error.response.data.message}`,
        {
  ...
  const getquizzes = async () => {
    const res = await axios.post("/api/administrator/candidates/quizzes/get", {
  ...
  const handleCargoCandidate = async (idCandidato: string) => {
    const res = await axios.post("/api/enterprise/candidate/", {
      id: idCandidato,
    });
    if (res.status == 200) {
  ...
  const updateQuiz = async (idQuiz: string) => {
    ...
    const res = await axios.post(
      "/api/administrator/candidates/quizzes/update",
      { idQuiz: idQuiz }
    );
    if (res.status == 200)
      setQuiz(res.data.quiz.preguntas),
        setShow(false),
        setCargoDeseado(res.data.cargoDeseado),
        setCandidato(res.data.idCandidato);
  };

  const deleteQuiz = async (idQuiz: string) => {
    try {
      const res = await axios.post(
        "/api/administrator/candidates/quizzes/delete",
        { idQuiz: idQuiz }
      );
              ...
              <tbody>
                {quizzData?.map((item: any, key: number) => (
                  <tr className="p-5" key={key}>
                    <td className="p-2">
                      {item.cuestionario.tituloCuestionario
                        ? item.cuestionario.tituloCuestionario
                        : "Sin especificar"}
                    </td>
                    <td className="p-2">
                    ...
                    <td className="p-2">
                      {item.candidatoData?.perfil?.puestoDeseado
                        ? item.candidatoData.perfil.puestoDeseado
                        : "Sin especificar"}
                    </td>
                    <td>
                      {item.cuestionario.estado
                        ? "Finalizado"
                        : "No completado"}
                    </td>
                    <td className="mt-2">
                      <button
                        onClick={() => updateQuiz(item._id)}
  ...
  useEffect(() => {
    if (!cargoDeseadoCandidato) {
      (async () => {
        await handleCargoCandidate(id);
      })();
    }
  }, [cargoDeseadoCandidato]);
        ...
        {isLoading ? (
          ...
          <>
            ...
            <div className="col-md-6">
              ...
              <select
                onChange={(e: any) => setDificultad(e.target.value)}
            ...
            <div className="col-md-6">
              ...
              <select
                onChange={(e: any) => setTypeQuiz(e.target.value)}
                className="form-control"
              >
            ...
            <div className="col-md-6 mt-3">
              <button
                ...
                className="text-white px-4 py-2 rounded-md mb-4 ml-5 bg-green-500"
              >
        ...
        {isLoading ? (
          ...
          <>
              ...
              <div className="row">
                ...
                <div className="col-md-12">
                  ...
                  <input
                    type="text"
                    className="form-control"
                    placeholder="Especifique el titulo con el que se diferenciara de los demas cuestionarios generados"
                    {...register("tituloCuestionario", { required: true })}
                  />
                </div>
                {cargoDeseadoCandidato == null ? (
                  <div className="col-md-12">
                    <label className="labels p-2">CargoDeseado:</label>
                    <p>
                      Al rellenar el cargo a certificar. Presione de nuevo
                      generar cuestionario
                    </p>
                    <input
                      type="text"
                      className="form-control"
                      placeholder="Especifique el cargo deseado con el que se generará el cuestionario"
                      {...register("cargoDeseado", { required: true })}
                    />
                      ...
                      <div key={field.id}>
                        <section className={"row"} key={field.id}>
                          ...
                          <div className="col-md-6">
                            ...
                            <textarea
                              className="form-control  w-full p-3 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                              {...register(`quiz.${index}.pregunta` as const, {
                                required: true,
                              })}
                          ...
                          <div className="col-md-6">
                            ...
                            <textarea
                              className="form-control  w-full p-3 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                              {...register(
                                `quiz.${index}.respuestaCorrecta` as const,
                                {
                                  required: true,
                                }
                              )}
                          ...
                          <div className="col-md-3">
                            <button
                              type="button"
                              className="btn mt-3 btn btn-danger "
            ...
            >
              ...
              <Modal.Body>
                <div className="row">{form(modalTitle)}</div>
              </Modal.Body>
            </Modal>
          </>
        ) : (
