...
export const FormStepper = (props: any) => {
  ...
  const sendData = async () => {
    try {
      const response = await axios.post("/api/candidate/quizzes/save", {
        respuestasCandidatos: respuestasCandidatos,
  ...
  return (
    <>
      <form
        ...
        className="p-5 space-y-6  w-full flex   justify-center items-center"
      >
        {data?.map((item: any, key: number) => {
          return (
            <div className="form text-center " key={key}>
              {step === item.page && (
                <div
                  className="bg-white p-4 sm:p-6 rounded-lg shadow-md max-w-full mx-auto"
                  key={key}
                >
                  <div className="mb-4">
                  ...
                  <div className="space-y-3">
                    {item.respuestas?.length === 0 ? (
                      ...
                      item.respuestas.map(
                        (respuestas: any, keyField: number) => {
                          return (
                            <div
                              key={keyField}
            ...
            {respuestasCandidatos?.map((item: any, key: number) => {
              return (
                <div key={key} className="space-y-2">
                  <p className="font-bold">{item.pregunta}</p>
